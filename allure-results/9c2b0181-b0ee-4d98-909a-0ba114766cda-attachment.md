...
Expected value: "qedaugrvhj"
Received value: "qedaugrvhj"
```
...
```ts
  ...
  35 | 
  36 |     expect(responseBody.booking).toHaveProperty("firstname", firstName);
  37 |     expect(responseBody.booking).toHaveProperty("lastname", lastName);
  38 |     expect(responseBody.booking.bookingdates).toHaveProperty("checkin", checkInDate);
  39 |     expect(responseBody.booking.bookingdates).toHaveProperty("checkout", checkOutDate);
> 40 |     expect(responseBody.booking.additionalneeds).toHaveProperty("additionalneeds", additionalNeeds);
     |                                                  ^ Error: expect(received).toHaveProperty(path, value)
  41 | 
  42 | 
  43 | 
