...
test("get booking details based on parameters", async({request})=>{
    ...
    const putResponse = await request.put(`${BASE_URL}/booking/${booking_id}`, {
    ...
    expect(putResponseBody).toHaveProperty("firstname", updateBookingPayload.firstname);
    expect(putResponseBody).toHaveProperty("lastname", updateBookingPayload.lastname);
