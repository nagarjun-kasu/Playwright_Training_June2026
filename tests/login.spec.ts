...
test("validate the login functionality", async({page})=>{
    ...
    await page.getByLabel("Password").filter({visible:true}).fill("Test@123");
    ...
    await expect(page.locator("#loginMsg")).toHaveText("Login successful! Welcome, nagarjun.");
    await expect(page.locator("#loginMsg")).toContainText("successful!");
